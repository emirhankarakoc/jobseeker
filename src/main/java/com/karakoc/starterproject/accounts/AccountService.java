...
public class AccountService {
    ...
    public UserDTO register(RegisterRequest request) {
        Optional<User> testUser3 = repository.findByMail(request.getMail());
        Optional<User> testUser2 = repository.findByPhoneNumber(request.getPhoneNumber());
        Optional<User> testUser = repository.findByUsername(request.getUsername());
        if (testUser.isPresent() || testUser2.isPresent() || testUser3.isPresent()) {
